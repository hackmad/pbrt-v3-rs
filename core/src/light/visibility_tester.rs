...
/// VisibilityTester allows lights to return a radiance value under the assumption that the reference point and light
/// source are mutually visible.
#[derive(Clone)]
...
impl VisibilityTester {
    ...
    /// Traces a shadow ray between `p0` and `p1` through the scene and returns true if the points are visible to each
    /// other.
    ///
    /// * `scene` - The scene.
    ...
    /// Computes the beam transmittance, the fraction of radiance transmitted along the segment between the two points.
