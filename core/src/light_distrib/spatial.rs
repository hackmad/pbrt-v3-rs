...
/// Voxel coordinates are packed into a uint64_t for hash table lookups; 10 bits
/// are allocated to each coordinate.  invalidPackedPos is an impossible packed
/// coordinate value, which we use to represent
...
impl SpatialLightDistribution {
    ...
    // Compute the sampling distribution for the voxel with integer coordiantes
    // given by "pi".
...
impl LightDistribution for SpatialLightDistribution {
    ...
    fn lookup(&self, p: &Point3f) -> Option<Arc<Distribution1D>> {
        ...
        // take packedPos mod the hash table size, but since packedPos isn't
        // necessarily well distributed on its own, it's worthwhile to do a little
        // work to make sure that its bits values are individually fairly random.
        ...
        loop {
            ...
            if entry_packed_pos == packed_pos {
                ...
                // The hash table entry we're checking has already been allocated
                // for another voxel. Advance to the next entry with quadratic
                // probing.
                hash += step * step;
                ...
                // We have found an invalid entry. (Though this may have changed
                // since the load into entryPackedPos above.) Use an atomic
                // compare/exchange to try to claim this entry for the current
                // position.
