...
impl Bounds3f {
    /// 3-D bounding box where minimum and maximum bounds are maximum and minimum floating point values. This is so we
...
impl<T: Num + Copy> Bounds3<T> {
    /// Creates a new 3-D bounding box from 2 points. The minimum and maximum bounds are used for each coordinate axis.
    ///
    /// * `p1` - First point.
    /// * `p2` - Second point.
    ...
    /// Returns a 3-D bounding box where minimum and maximum bounds are maximum and minimum values respectively of the
    /// type's limits. This is so we can easily grow the bounding box from nothing iteratively.
    pub fn empty() -> Self
    where
    ...
    /// corner has offset `(0, 0, 0)` and a point at the maximum corner has offset is `(1, 1, 1)`.
    ///
    /// * `p` - The point.
    ...
    /// Return the center and radius of a sphere bounded on the corners of the bounding box.
    ...
    /// Returns the near and far ray parameters where it intersects the bounding box. If no intersection occurs `None`
    /// is returned.
    ///
    /// * `ray` - The ray
    ...
    /// Uses the reciprocal of a rays direction and returns `true` if it intersects the bounding box; otherwise `false`.
    ///
    /// * `ray`        - The ray.
