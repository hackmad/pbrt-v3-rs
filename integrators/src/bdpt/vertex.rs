...
/// Vertex represents any kind of path vertex and encapsulates conditional logic to handle different cases between
/// pairs of vertices.
#[derive(Clone)]
pub(crate) struct Vertex<'scene> {
    ...
    /// Tracks β which is the product of the BSDF or phase function values, transmittances, and cosine terms for the
    /// vertices in the path generated, divided by their respective sampling PDFs.
    ...
    /// Used by surface interactions and records whether a Dirac delta function was sampled (e.g., when light is
    /// scattered by a perfectly specular material).
    pub(crate) delta: bool,

    /// Forward density of the current vertex, which is the probability per unit area of the current vertex as
    /// generated by the path sampling algorithm.
    pub(crate) pdf_fwd: Float,

    /// The hypothetical probability density of the vertex if the direction of light transport was reversed—that is,
    /// if radiance transport was used in place of importance transport for the camera path and vice versa for the light
    /// path. Used for computing MIS weights.
...
impl<'scene> Vertex<'scene> {
    ...
    /// * `beta`        - Tracks β which is the product of the BSDF or phase function values, transmittances, and cosine
    ...
    /// * `beta` - Tracks β which is the product of the BSDF or phase function values, transmittances, and cosine terms
    ///            for the vertices in the path generated, divided by their respective sampling PDFs.
    ...
    /// * `le`      - Used for β which is the product of the BSDF or phase function values, transmittances, and cosine
    ///               terms for the vertices in the path generated, divided by their respective sampling PDFs.
    /// * `pdf`     - The light PDF.
    ...
    /// Returns true if the vertex is a light endpoint interaction or a delta light (has a Dirac delta distribution).
    ...
    /// Returns true if the vertex is a light endpoint interaction, an infinite light source or a delta direction light
    /// source.
    ...
    /// Returns the product of this Jacobian (computed from the vertex attributes) and the pdf parameter, which should
    /// express a solid angle density at the vertex.
    ///
    /// * `pdf`  - The PDF.
    /// * `next` - The next vertex in the path.
    ...
    /// Returns the probability per unit area for path endpoints whose sampling routines generate rays from a 4D
    /// distribution.
    ///
    /// * `scene`                - The scene.
    /// * `v`                    - The vertex.
