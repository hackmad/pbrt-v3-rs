...
/// Implements a “kitchen sink” material representing the union of many of the preceding materials. This is a highly
/// parameterized material that is particularly useful when converting scenes from other file formats into pbrt’s.
pub struct UberMaterial {
    ...
    /// The opacity of the surface. Note that when less than one, the uber material transmits light without refracting
    /// it.
...
impl UberMaterial {
    ...
    /// * `opacity`         - The opacity of the surface. Note that when less than one, the uber material transmits
    ...
    ///                       modeled.
    /// * `index`           - The index of refraction of the inside of the object. Implicitly assumes that the exterior
    ///                       of objects is a vacuum, with IOR of 1.
...
impl Material for UberMaterial {
    ...
    fn compute_scattering_functions<'scene>(
        ...
        let op = self.opacity.evaluate(&si.hit, &si.uv, &si.der).clamp_default();
...
impl From<&TextureParams> for UberMaterial {
    ...
    fn from(tp: &TextureParams) -> Self {
        ...
        let kr = tp.get_spectrum_texture_or_else("Kr", Spectrum::ZERO, |v| Arc::new(ConstantTexture::new(v)));
        let kt = tp.get_spectrum_texture_or_else("Kt", Spectrum::ZERO, |v| Arc::new(ConstantTexture::new(v)));
        ...
        let opacity = tp.get_spectrum_texture_or_else("opacity", Spectrum::ONE, |v| Arc::new(ConstantTexture::new(v)));
