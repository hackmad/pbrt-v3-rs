...
#![allow(dead_code)]
use super::{
    create_triangle_mesh, ArcShape, ArcTransform, Float, Normal3, Normal3f, Point3f, Vector3,
    Vector3f, PI, TWO_PI,
};
...
fn sd_vertex(id: usize, p: &Point3f) -> SDVertex {
    SDVertex {
        id,
        p: *p,
        start_face: None,
...
/// * `v0` - First endpoint.
/// * `v1` - Second endpoint.
fn sd_edge(v0: usize, v1: usize) -> SDEdge {
    SDEdge {
        v: [v0, v1],
        f: [None, None],
        f0_edge_num: None,
...
pub fn loop_subdivide(
    ...
    let mut vertices: Vec<Arc<SDVertex>> = p
        .iter()
        .enumerate()
        .map(|(id, point)| Arc::new(sd_vertex(id, &point)))
        .collect();
    ...
    for i in 0..n_faces {
        for edge_num in 0..3 {
            ...
            let mut e = sd_edge(faces[i].v[v0], faces[i].v[v1]);
    ...
    for level in 0..n_levels {
        ...
        for face in faces.iter() {
            for k in 0..3 {
                ...
                let edge = sd_edge(face.v[k], face.v[next!(k)]);
        ...
        for face in faces.iter() {
            for j in 0..3 {
                ...
                let edge = sd_edge(face.v[j], face.v[next!(j)]);
