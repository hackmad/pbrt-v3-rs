...
pub fn coordinate_system<T: Float>(v1: &Vector3<T>, v2: &mut Vector3<T>, v3: &mut Vector3<T>) {
    if abs(v1.x) > abs(v1.y) {
        *v2 = vector3(-v1.z, T::zero(), v1.x) / (v1.x * v1.x + v1.z * v1.z).sqrt();
    } else {
        *v2 = vector3(T::zero(), v1.z, -v1.y) / (v1.y * v1.y + v1.z * v1.z).sqrt();
    };
    *v3 = v1.cross(v2);
}
...
mod tests {
    ...
    fn from_unit_x_axis() {
        ...
        let mut v2 = vector3(0.0, 0.0, 0.0);
        let mut v3 = vector3(0.0, 0.0, 0.0);
        let cs = coordinate_system(&v1, &mut v2, &mut v3);
        assert!(v2 == vector3(0.0, 0.0, 1.0));
        assert!(v3 == vector3(0.0, -1.0, 0.0));
    }
